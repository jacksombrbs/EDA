...
async function montarHtmlListaFrequencia(idCurso, idDisciplina, data) {
    ...
    participantes.forEach(participante => {
        html += `<tr class="linha-assinatura"><td><strong>${Utilidades.escaparHtml(participante.nome || '-')}</strong></td><td>${Utilidades.escaparHtml(mapaParoquias[participante.id_paroquia] || 'Não informada')}</td><td></td></tr>`;
    });
    html += '</tbody></table>';
