...
const Utilidades = {
    ...
    notificacao(mensagem, tipo = 'sucesso') {
        ...
        div.className = `pos-fixa direita-30 base-30 p-md raio-sm sombra-2 texto-lg peso-bold z-maximo fundo-toast-${tipo} cor-texto-${tipo} animacao-deslize`;
...
const Interface = {
    ...
    criarTabela(colunas, dados, aoClique) {
        let codigoEstrutura = '<div class="rolagem-x borda-1 borda-solida borda-cor-padrao raio-md fundo-branco mb-lg"><table class="w-total borda-colapso texto-md"><thead><tr>';

        colunas.forEach(coluna => {
            codigoEstrutura += `<th class="p-md texto-esquerda peso-bold cor-texto-primario fundo-cinza borda-base-2 borda-solida borda-cor-padrao">${coluna}</th>`;
        });
        codigoEstrutura += '<th class="p-md texto-esquerda peso-bold cor-texto-primario fundo-cinza borda-base-2 borda-solida borda-cor-padrao">Ações</th></tr></thead><tbody>';

        dados.forEach((linha, indice) => {
            codigoEstrutura += '<tr>';
            Object.values(linha).forEach(valor => {
                codigoEstrutura += `<td class="p-md texto-esquerda borda-base-1 borda-solida borda-cor-padrao">${valor || '-'}</td>`;
            });
            codigoEstrutura += `<td class="flex gap-sm p-md texto-esquerda borda-base-1 borda-solida borda-cor-padrao">
                <button class="inline-flex itens-centro justifica-centro gap-sm p-sm px-md raio-xxs texto-sm peso-regular transicao sem-borda cursor-apontador cor-texto-escuro fundo-superficie-2 hover-fundo-superficie-3" onclick="aoClique('editar', ${indice})">Editar</button>
                <button class="inline-flex itens-centro justifica-centro gap-sm p-sm px-md raio-xxs texto-sm peso-regular transicao sem-borda cursor-apontador cor-texto-escuro fundo-superficie-2 hover-fundo-superficie-3" onclick="aoClique('excluir', ${indice})">Excluir</button>
            </td></tr>`;
        });
    ...
    criarCampoBusca(id, textoExemplo = 'Buscar...') {
        return `
            <div class="flex mb-md">
                <input type="text" id="${id}" class="w-total p-sm px-md min-h-44 fundo-cinza borda-2 borda-solida borda-cor-padrao raio-sm cor-texto-escuro texto-md transicao focus-sem-outline focus-borda-marca" placeholder="${textoExemplo}">
            </div>
        `;
    }
};
    ...
    let codigoEstrutura = `
        <div class="flex flex-coluna mb-md pos-relativa w-total">
            ...
            <div class="cursor-apontador sem-selecao pos-relativa" onclick="window.alternarSeletorCustomizado(this, event)">
                <div class="w-total p-sm px-md min-h-44 fundo-cinza borda-2 borda-solida borda-cor-padrao raio-sm cor-texto-escuro texto-md transicao focus-sem-outline focus-borda-marca flex itens-centro justifica-espaco" id="acionador-${id}">
                    <span>${textoSelecionado}</span>
                    <span class="texto-sm cor-texto-claro">▼</span>
                </div>
                <div class="opcoes-seletor-customizado pos-absoluta fundo-branco borda-1 borda-solida borda-cor-padrao raio-sm rolagem-y z-maximo w-total oculto" style="max-height: 200px; top: 100%; margin-top: 4px;">
...
function criarCampoFormulario(rotulo, tipo, id, valor = '', textoExemplo = '', obrigatorio = false) {
    ...
    return `
        <div class="flex flex-coluna mb-md">
            ...
            <input type="${tipo}" id="${id}" class="w-total p-sm px-md min-h-44 fundo-cinza borda-2 borda-solida borda-cor-padrao raio-sm cor-texto-escuro texto-md transicao focus-sem-outline focus-borda-marca" value="${valor}" placeholder="${textoExemplo}" ${obrigatorioHtml}>
        </div>
    `;
}
...
function criarAreaTexto(rotulo, id, valor = '', linhas = 3, obrigatorio = false) {
    ...
    return `
        <div class="flex flex-coluna mb-md" style="grid-column: 1 / -1;">
            <label for="${id}" class="peso-bold mb-sm cor-texto-escuro texto-md">${rotulo}</label>
            <textarea id="${id}" class="w-total p-sm px-md min-h-44 fundo-cinza borda-2 borda-solida borda-cor-padrao raio-sm cor-texto-escuro texto-md transicao focus-sem-outline focus-borda-marca" rows="${linhas}" ${obrigatorioHtml}>${valor}</textarea>
        </div>
    `;
}
